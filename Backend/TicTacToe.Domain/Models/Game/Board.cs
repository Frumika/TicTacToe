...
namespace TicTacToe.Domain.Models.Game;
...
public class Board
{
    ...
    {
        FieldItem item = FieldItem.Empty;
    ...
    {
        if (item == FieldItem.Empty) return false;
    ...
    public bool IsWinningField(int row, int column)
    {
        ...
        if (field.Item == FieldItem.Empty) return false;
    ...
    public void ResetBoard()
    {
        for (int i = 0; i < Rows; i++)
        {
            for (int j = 0; j < Columns; j++)
            {
                _board[i, j].Item = FieldItem.Empty;
            }
        }
